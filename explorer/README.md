...
The source for `site/index.html`: eight tabs, built to a single
...
CI builds it and publishes `site/` to GitHub Pages on every merge to `main`; locally,
`pnpm serve` builds it for you.
...
1. **`pnpm facts`** loads the domain and gateway configs — the same files the
...
One level up is the repository root, which builds all this: `scripts/` holds the pipeline,
and `scripts/lib/paths.ts` is the only place that knows where the FLEX checkout is or where the
built site goes.

The page has to stay single-file: it is served straight from the site folder and published as
...
```bash
pnpm exec eslint --fix explorer/model/<file>.json
```
...
2. `pnpm exec likec4 validate explorer/model` —
...
pnpm exec likec4 start explorer/model
...
1. **`explorer.config.json`** — rewrite it. Name, kinds, stages, repo URL, and the `source`
   block naming the checkout to document and the files to read from it. `site` says where the
   built page lands.
...
4. **`architectureFacts.ts`** — this one cannot be reused as-is. It reads
   ...
   is closer to portable — it parses CDK constructs from the AST and needs no install — but
   still knows what a CloudWatch alarm looks like. Write your own against your own configs,
   or delete both, leave `from` off every resource and `derived` off every table, and
   maintain those numbers by hand: each check skips when nothing declares a source.
